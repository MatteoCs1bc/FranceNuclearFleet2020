...
def palier_summary(hourly_by_reactor: dict, reactors: list, date_from, date_to,
                   meta_df) -> pd.DataFrame:
    """
    Aggrega le performance per PALIER (blocco di generazione): CF medio,
    velocità e profondità di modulazione medie, cicli max, capacità totale.
    """
    from src import loadfollowing as lf
    from src.metadata import enrich_reactor_list
    enr = enrich_reactor_list(reactors, meta_df)
    info = {r["reactor"]: r for _, r in enr.iterrows()}
    ...
    for r in reactors:
        ...
        rows.append({
            ...
            "net_MW": m["net_MW"],
            "cf": k["cf_nominal"],
            "ramp_max": ml["max_ramp_rate_pct_h"],
            "depth": ml["max_depth_pct"],
            "cycles": ml["max_cycles_day"],
            "baseload": ml["pct_baseload"],
        })
    if not rows:
        return pd.DataFrame()
    df = pd.DataFrame(rows)
    g = df.groupby("palier").agg(
        n_reactors=("cf", "size"),
        total_GW=("net_MW", lambda x: x.sum() / 1000),
        mean_cf=("cf", "mean"),
        mean_ramp_max=("ramp_max", "mean"),

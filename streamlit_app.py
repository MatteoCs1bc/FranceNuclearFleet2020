...
def _find_repo_data():
    ...
    Ritorna ('zip', path) | ('folder', path) | (None, None).
    Priorità: ZIP in data/ → ZIP nella root → CSV in data/.
    """
...
def sidebar_controls():
    ...
    kind, path = _find_repo_data()
    if kind == "zip":
        with st.spinner(f"Carico {path.name}…"):
            prod, unavail, hourly, nominal, errors = _load_zip_path_cached(
                str(path), path.stat().st_mtime
            )
        st.sidebar.success(f"📦 Dati dal repo: `{path.name}`")
    elif kind == "folder":
        with st.spinner("Carico i CSV da ./data…"):
            prod, unavail, hourly, nominal, errors = _load_folder_cached(str(path))
        st.sidebar.success(f"📁 Dati dal repo: `{path}/`")

    # 2) Override facoltativo (caricare un altro ZIP al volo)
    with st.sidebar.expander("📂 Usa un altro file" + ("" if hourly else " (nessun dato nel repo)"),
                             expanded=not bool(hourly)):
        if not hourly:
            st.caption(
                "Nessuno ZIP/CSV trovato in `data/`. Metti lo ZIP nel repo "
                "(cartella `data/`) oppure caricalo qui."
            )
        ...
        if zf is not None:
            ...
            st.success("Uso il file caricato manualmente.")

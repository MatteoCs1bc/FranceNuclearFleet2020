...
def render_single(reactor, hourly, unavail_data, date_from, date_to):
    ...
    # --- MODULAZIONE (focus) ---
    with tabs[0]:
        st.markdown("#### Inviluppo delle manovre di modulazione")
        st.caption(
            "Ogni punto è una manovra mentre il reattore è in marcia: durata (ore) "
            "× ampiezza (% Pnom). La stella è la manovra massima. Un reattore "
            "nucleare sta in basso-a-sinistra (manovre piccole e lente); una turbina "
            "a gas riempirebbe tutto il quadrante. Qui si vedono i **limiti fisici**."
        )
        st.plotly_chart(charts.lf_ramp_envelope(h), use_container_width=True)

        st.markdown("#### Firma giornaliera: quando e quanto modula")
        st.caption(
            "Produzione media e rampe per ora del giorno. L'avvallamento di "
            "mezzogiorno, se presente, è indotto dal fotovoltaico."
        )
        st.plotly_chart(charts.lf_diurnal_profile(h), use_container_width=True)

        # riga di sintesi testuale del confronto col gas
        st.info(
            f"**In sintesi** — {reactor} nel periodo: manovra al massimo a "
            f"{ml['max_ramp_rate_pct_h']:.0f}% Pnom/h, scende fino a −{ml['max_depth_pct']:.0f}%, "
            f"al più {ml['max_cycles_day']} cicli/giorno, con ~{ml['median_gap_h']:.0f} h di stabilità "
            f"tra una manovra e l'altra. Sta a piena potenza il {ml['pct_baseload']:.0f}% del tempo. "
            f"Una CCGT a gas farebbe rampe più ampie, più rapide e più frequenti, senza tempi di recupero."
            if pd.notna(ml['median_gap_h']) else
            f"**In sintesi** — {reactor}: rampa max {ml['max_ramp_rate_pct_h']:.0f}% Pnom/h, "
            f"profondità −{ml['max_depth_pct']:.0f}%, baseload {ml['pct_baseload']:.0f}% del tempo."
        )
...
def render_fleet(selected, hourly, nominal, date_from, date_to):
    ...
    # KPI flotta: focus su modulazione simultanea
    ml = lf.modulation_limits(fleet)
    mat = pd.concat(
        [(lf.classify_states(slice_period(hourly[r], date_from, date_to)) == "load_follow").astype(int)
         for r in selected if r in hourly and not slice_period(hourly[r], date_from, date_to).empty],
        axis=1,
    ).fillna(0) if selected else pd.DataFrame()
    simul = mat.sum(axis=1) if not mat.empty else pd.Series([0])

    c1, c2, c3, c4, c5 = st.columns(5)
    c1.metric("Capacità installata", f"{kpi['installed_MW']/1000:.1f} GW")
    c2.metric("Max reattori modulanti insieme", f"{int(simul.max())}/{len(selected)}",
              help="Picco di reattori simultaneamente in load-following")
    c3.metric("In media modulano", f"{simul.mean():.0f}")
    c4.metric("CF flotta", f"{kpi['cf_nominal']:.1f}%")
    c5.metric("Energia", f"{kpi['energy_produced_TWh']:.1f} TWh")

    st.divider()
    tabs = st.tabs(["🔁 Modulazione flotta", "🏭 Limiti per palier & anagrafica",
                    "📊 Confronto reattori"])
    ...
    with tabs[0]:
        ...
        st.caption(
            f"Su {len(selected)} reattori selezionati, al più **{int(simul.max())}** hanno "
            "modulato nella stessa ora. È il limite pratico di flessibilità simultanea del parco."
        )
        ...
        st.caption("La modulazione della flotta insegue la domanda netta "
                   "(domanda − solare/eolico): l'avvallamento di mezzogiorno è il fotovoltaico.")
        st.plotly_chart(charts.lf_diurnal_profile(fleet), use_container_width=True)

        st.info(
            f"**Limiti di flotta** — max {int(simul.max())}/{len(selected)} reattori modulanti insieme; "
            f"rampa aggregata max {ml['max_ramp_rate_pct_h']:.0f}% dell'installato/h; "
            f"la flotta sta a piena potenza il {ml['pct_baseload']:.0f}% del tempo. "
            "La modulazione non è libera come per un parco a gas: è vincolata dal numero di "
            "reattori manovrabili insieme e dai loro tempi di recupero."
        )

    # --- LIMITI PER PALIER & ANAGRAFICA ---
    with tabs[1]:
        ...
        st.markdown("#### Velocità di modulazione per palier")
        st.caption("Boxplot per generazione di progetto: i palier più recenti e i 900 MW "
                   "modulano più velocemente dei P4 più vecchi.")
        fig = charts.palier_modulation_box(mod, enriched, "ramp_rate_pct", "% Pnom/h")
        if fig:
            st.plotly_chart(fig, use_container_width=True)
    ...
    # --- CONFRONTO REATTORI ---
    with tabs[2]:
        st.markdown("#### Ranking capacity factor")
        ...
        for r in selected:
            ...
            if h.empty:
                continue
            k = compute_kpis(h)
            m = lf.modulation_limits(h)
            e = einfo.get(r, {})
            rows.append({
                "Reattore": r,
                "Palier": e.get("palier", "—"),
                "Anno": int(e["commissioning_year"]) if pd.notna(e.get("commissioning_year")) else None,
                "CF %": round(k["cf_nominal"], 1),
                "Rampa max %/h": round(m["max_ramp_rate_pct_h"], 0),
                "Profondità %": round(m["max_depth_pct"], 0),
                "Max cicli/gg": m["max_cycles_day"],
                "Baseload %": round(m["pct_baseload"], 0),
            })
        if rows:
